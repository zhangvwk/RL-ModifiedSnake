...
def TransformQuadrantBasedOnDirection(coord, direction):
  ...
  if direction == Directions.LEFT:  (x, y) = ( y, -x)
  if direction == Directions.RIGHT: (x, y) = (-y,  x)
  if direction == Directions.DOWN:  (x, y) = (-x, -y)
...
class QuadrantView:
  """ 
    The Quadrant View class maps the snake board as follows:
  ...
  def __SquareDescription(self, snakeCoord_X, snakeCoord_Y, applePos, blockPos, square):
    ...
    if square in blockPos: return -1
    if square in snakeCoord: return -1

    (x, y) = square
    if x < 0 or windowWidth - pixel < x: return -1
    if y < 0 or windowHeight - pixel < y: return -1

    if (x, y) == applePos: return 1
    return 0
